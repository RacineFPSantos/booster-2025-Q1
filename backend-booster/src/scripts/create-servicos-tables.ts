import { AppDataSource } from '../core/database/typeorm.config';

async function createServicosTables() {
  try {
    await AppDataSource.initialize();
    const queryRunner = AppDataSource.createQueryRunner();

    console.log('Creating tipo_servico table...');
    ...
    console.log('Creating servico table...');
    await queryRunner.query(`
      CREATE TABLE IF NOT EXISTS servico (
        ...
        created_at TIMESTAMP WITHOUT TIME ZONE NOT NULL DEFAULT NOW(),
        updated_at TIMESTAMP WITHOUT TIME ZONE NOT NULL DEFAULT NOW(),
        CONSTRAINT fk_servico_tipo FOREIGN KEY (id_tipo_servico)
          REFERENCES tipo_servico(id_tipo_servico) ON DELETE RESTRICT
      );
    `);

    console.log('Creating agendamento table...');
    await queryRunner.query(`
      CREATE TABLE IF NOT EXISTS agendamento (
        ...
        CONSTRAINT fk_agendamento_usuario FOREIGN KEY (id_usuario)
          REFERENCES usuario(id_usuario) ON DELETE SET NULL
      );
    `);

    console.log('Inserting tipo_servico data...');
    await queryRunner.query(`
    ...
    console.log('Inserting servico data...');
    await queryRunner.query(`
